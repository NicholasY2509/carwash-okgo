...
interface Props {
    tier?: StaffIncentiveTier | null;
    onSuccess: () => void;
    onCancel: () => void;
}

export default function StaffIncentiveTierForm({ tier, onSuccess, onCancel }: Props) {
    ...
    const { data, setData, post, patch, processing, errors, reset, transform } = useForm({
        ...
        flat_amount: tier?.flat_amount?.toString() || "0",
    });
    ...
    }, [data.max_cars, data.flat_amount]);

    function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        e.preventDefault();

        if (isEditMode) {
            patch(route("staff-incentive-tiers.update", tier.id), {
                onSuccess: () => {
            ...
            post(route("staff-incentive-tiers.store"), {
                onSuccess: () => {
                    reset();
    ...
    return (
        <form onSubmit={handleSubmit}>
            <fieldset disabled={processing} className="space-y-4">
                ...
                <div>
                    <Label htmlFor="flat_amount" required>
                        Nominal Insentif
                    </Label>
                    <NumericFormat
                        id="flat_amount"
                        ...
                        onValueChange={(values) => setData("flat_amount", values.floatValue?.toString() || "")}
                        className="mt-1"
                        required
                    />
                    {errors.flat_amount && (
                        <p className="text-sm text-red-600 mt-1">{errors.flat_amount}</p>
                    )}
                </div>
            </fieldset>
