...
export default function StaffLevelIncentivesIndex() {
    const { props } = usePage<any>();
    const staffReport = (props.staffReport as Staff[]) || [];
    const tiers = (props.tiers as StaffIncentiveTier[]) || [];
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: "Insentif Staff", href: "/staff-level-incentives" },
    ];

    const handleFilter = () => {
        router.get(
            route("staff-level-incentives.index"),
        ...
        },
        {
            ...
            cell: (info) => (
                <div className="font-bold text-primary text-base">
                    {info.getValue() as number} <span className="text-xs font-normal text-muted-foreground">Mobil</span>
                </div>
            ),
        },
        {
            ...
            <div className="flex h-full flex-1 flex-col space-y-6 rounded-xl p-4 lg:p-6">
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <Heading
                        ...
                        description="Laporan detail hasil cuci mobil per staff dan insentif flat berdasarkan target tier."
                    />
                    ...
                    <motion.div variants={cardVariants}>
                        <Card className="shadow-sm border bg-gradient-to-tr from-primary/5 via-card to-card hover:shadow-md transition-shadow">
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-sm font-semibold">Total Mobil Dicuci</CardTitle>
                                <Car className="h-5 w-5 text-primary" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-3xl font-extrabold text-primary">{totalWashes} Unit</div>
                    ...
                    <motion.div variants={cardVariants}>
                        <Card className="shadow-sm border bg-gradient-to-tr from-emerald-500/5 via-card to-card hover:shadow-md transition-shadow">
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                    Akumulasi pengeluaran insentif staff
                                </p>
                            </CardContent>
                        </Card>
                    </motion.div>
                    ...
                    <motion.div variants={cardVariants}>
                        <Card className="shadow-sm border bg-gradient-to-tr from-blue-500/5 via-card to-card hover:shadow-md transition-shadow">
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground mt-1">
                                    Total pemasukan dari cucian staff
                                </p>
                            </CardContent>
                        </Card>
                    </motion.div>
                                    ...
                                    {tiers.length > 0 ? (
                                        tiers.map((t) => (
                                            <div key={t.id} className="flex justify-between text-xs items-center">
                                                <span className="font-semibold text-muted-foreground">{t.name} ({t.max_cars ? `${t.min_cars}-${t.max_cars}` : `>${t.min_cars}`} mobil)</span>
                                                <span className="font-bold text-emerald-600">{formatToIDR(t.flat_amount)} flat</span>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="flex items-center gap-1 text-xs text-amber-600">
                                            <Info className="h-3 w-3" />
                                            Belum ada tier. Atur di Master Insentif Staff.
                                        </div>
                                    )}
                                </div>
                            </CardContent>
