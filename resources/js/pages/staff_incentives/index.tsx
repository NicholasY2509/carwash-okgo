    ...
    const columns: ColumnDef<Staff>[] = [
        ...
        ...tiers.map((tier) => ({
            ...
            cell: (row: any) => {
                ...
                return (
                    <span className={earned > 0 ? "font-semibold text-foreground text-sm" : "text-muted-foreground/40 text-sm"}>
                        {formatToIDR(earned)}
                    </span>
                );
            }
        })),
        {
            ...
            cell: (info) => {
                ...
                if (tierName.toLowerCase().includes("tier 1")) {
                    ...
                    badgeClass = "bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800";
                                    ...
                                    {tiers.length > 0 ? (
                                        tiers.map((t) => (
                                            <div key={t.id} className="flex justify-between text-xs items-center">
                                                ...
                                                <span className="font-bold text-emerald-600">{formatToIDR(t.commission)} / mobil</span>
                                            </div>
                                        ))
                                    ) : (
                                        <div className="flex items-center gap-1 text-xs text-amber-600">
                                            <Info className="h-3 w-3" />
                                            Belum ada tier. Atur di Master Insentif.
                                        </div>
                                    )}
                                </div>
                            </CardContent>
