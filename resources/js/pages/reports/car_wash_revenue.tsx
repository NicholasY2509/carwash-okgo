...
import { DollarSign, TrendingUp, Ticket, Banknote, Calendar, BarChart3 } from "lucide-react";
...
export default function CarWashRevenueReport() {
    ...
    useEffect(() => {
        ...
        router.get(route("reports.car-wash-revenue"), { report_type: reportType, start_date: startDate, end_date: endDate }, { preserveState: true });
    ...
    const handlePageChange = (page: number) => {
        router.get(route("reports.car-wash-revenue"), {
    ...
    const columns: ColumnDef<ReportRow>[] = filters.report_type === "daily" ? [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return <Badge variant={getVariant()} className="text-[10px] py-0.5 px-1.5">{type}</Badge>;
            },
        },
        {
        ...
        },
        {
            ...
            header: "Voucher",
            cell: ({ row }) => <span className="text-indigo-600 dark:text-indigo-400 font-medium">{formatRupiah(row.original.voucher_revenue || 0)}</span>,
        },
        {
            accessorKey: "transfer_revenue",
            header: "Transfer",
            cell: ({ row }) => <span className="text-amber-600 dark:text-amber-400 font-medium">{formatRupiah(row.original.transfer_revenue || 0)}</span>,
        },
        {
            accessorKey: "warranty_count",
            header: "Garansi",
            cell: ({ row }) => <div className="text-center text-xs font-semibold text-muted-foreground">{row.original.warranty_count}</div>,
        },
        {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col space-y-6 rounded-xl p-4 lg:p-6">
                ...
                <div className="flex flex-col gap-1">
                    <Heading title="Laporan Omset Cuci Mobil" description="Laporan omset pendapatan cuci mobil berdasarkan periode harian atau bulanan." />
                </div>
                ...
                <motion.div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3" variants={containerVariants} initial="hidden" animate="show">
                    <motion.div variants={cardVariants}>
                        <Card className="shadow-sm border bg-gradient-to-tr from-blue-500/10 via-card to-card hover:shadow-md transition-shadow">
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-sm font-semibold">Total Omset</CardTitle>
                    ...
                    <motion.div variants={cardVariants}>
                        <Card className="shadow-sm border bg-gradient-to-tr from-indigo-500/10 via-card to-card hover:shadow-md transition-shadow">
                            <CardHeader className="flex flex-row items-center justify-between pb-2">
                                <CardTitle className="text-sm font-semibold">Pembayaran Voucher</CardTitle>
                                <Ticket className="h-5 w-5 text-indigo-600" />
                            </CardHeader>
                            <CardContent>
                                <div className="text-2xl font-extrabold text-indigo-600 dark:text-indigo-400">{formatRupiah(summary.voucher_revenue || 0)}</div>
                                <p className="text-xs text-muted-foreground mt-1">Penukaran voucher</p>
                            </CardContent>
                        </Card>
                    </motion.div>
                    {/* <motion.div variants={cardVariants}>
