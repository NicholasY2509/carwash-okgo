...
interface SpecialVoucherTabProps {
    specialCodes: string[];
    currentSpecialCode: string;
    onCurrentSpecialCodeChange: (value: string) => void;
    onAddSpecialCode: () => void;
    onRemoveSpecialCode: (code: string) => void;
...
export default function SpecialVoucherTab({
    specialCodes,
    currentSpecialCode,
    onCurrentSpecialCodeChange,
    onAddSpecialCode,
    onRemoveSpecialCode,
    specialCategoryId,
    ...
    categories,
}: SpecialVoucherTabProps) {
    return (
        <div className="space-y-4">
            <div className="space-y-4">
                    <div>
                        <Label htmlFor="special-code" required>
                            Kode Voucher Khusus
                        </Label>
                        <div className="flex items-center gap-2">
                            <Input
                                id="special-code"
                                placeholder="e.g., SPECIAL123"
                                value={currentSpecialCode}
                                onChange={(e) => onCurrentSpecialCodeChange(e.target.value)}
                                onKeyDown={(e) =>
                                    e.key === "Enter" &&
                                    (e.preventDefault(), onAddSpecialCode())
                                }
                            />
                            <Button type="button" onClick={onAddSpecialCode}>
                                Tambah
                            </Button>
                        </div>
                        <div className="flex min-h-[60px] flex-wrap gap-2 rounded-md border p-2 mt-2">
                            {specialCodes.length === 0 ? (
                                <span className="p-2 text-sm text-muted-foreground">
                                    Kode voucher khusus yang ditambahkan akan muncul di sini...
                                </span>
                            ) : (
                                specialCodes.map((code) => (
                                    <Badge key={code} variant="secondary">
                                        ...
                                        <button
                                            ...
                                            onClick={() => onRemoveSpecialCode(code)}
                                        >
                                            <X size={12} />
                                        </button>
                                    </Badge>
